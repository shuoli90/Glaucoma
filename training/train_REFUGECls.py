...
def train_model(model, dataloaders, criterion, optimizer, num_epochs=25, is_inception=False, model_name='resnet50', feature_extract=True):
    since = time.time()
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and auc > best_auc:
                ...
                best_model_wts = copy.deepcopy(model.state_dict())
                if feature_extract:
                    torch.save(model.state_dict(), f'best_resnet_{model_name}_REFUGE.pt')
                else:
                    torch.save(model.state_dict(), f'best_resnet_{model_name}_REFUGE_fulltrain.pt')
            if phase == 'val':
    ...
    plt.title('AUC')
    if feature_extract:
        plt.savefig(f'AUC_{model_name}.png')
    else:
        plt.savefig(f'AUC_{model_name}_fulltrain.png')
...
if __name__ == '__main__':
    ...
    train_dataset = utils.REFUGEClsDataset(images=train_images, transform=data_transforms['train'])
    val_dataset = utils.REFUGEClsDataset(images=val_images, transform=data_transforms['val'], annotation=val_annotation, nottrain=True)
    test_dataset = utils.REFUGEClsDataset(images=test_images, transform=data_transforms['val'], annotation=test_annotation, nottrain=True)
    train_loader = DataLoader(train_dataset, batch_size=args.batch, shuffle=True,  pin_memory=True, num_workers=args.workers)
    val_loader = DataLoader(val_dataset, batch_size=args.batch, shuffle=True,  pin_memory=True, num_workers=args.workers)
    test_loader = DataLoader(test_dataset, batch_size=args.batch, shuffle=False, pin_memory=True, num_workers=args.workers)
    ...
    if feature_extract:
        model_ft.load_state_dict(torch.load(f'best_resnet_{args.model}_REFUGE.pt'))
    else:
        model_ft.load_state_dict(torch.load(f'best_resnet_{args.model}_REFUGE_fulltrain.pt'))
    model_ft.eval()
    phase = 'test'
    running_corrects = 0
