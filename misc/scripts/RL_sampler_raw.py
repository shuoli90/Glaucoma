...
class SelectEnv(gym.Env):
    ...
    def __init__(self, N_IMAGE, N_SELECTION, random_seed=42):
        ...
        self.observation_space = spaces.Box(low=0, high=255,
                                            shape=(3 * self.N_IMAGE, self.HEIGHT, self.WIDTH), dtype=np.uint8)
        ...
        self.indices = []
        self.selected = []
        self.reg = re.compile(r'P(\d+)_P(\d+)')
    ...
    def reset(self, train=True):
        ...
        if real_count < self.N_IMAGE:
            for i in range(self.N_IMAGE - real_count):
                ...
                observation.append(torch.zeros(3, self.HEIGHT, self.WIDTH))
...
if __name__ == "__main__":
    ...
    model = PPO.load("./models/RL/sample_selection_previous")
